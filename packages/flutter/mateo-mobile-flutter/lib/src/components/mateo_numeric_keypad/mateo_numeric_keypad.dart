...
import 'package:mateo_mobile_old/src/components/mateo_numeric_keypad/mateo_numeric_keypad_key.dart';
import 'package:mateo_mobile_old/src/components/mateo_numeric_keypad/mateo_numeric_keypad_variant.dart';
import 'package:mateo_mobile_old/src/components/mateo_tap/mateo_tap.dart';
import 'package:mateo_mobile_old/src/components/mateo_text/mateo_text_field.dart';
...
/// and the edit. Configure each corresponding [MateoTextField] with
...
/// ```dart
/// Widget buildKeypad({
///   required MateoTextController controller,
...
class _MateoNumericKeypadState extends State<MateoNumericKeypad> {
  static const _numberFontSize = 24.0;
  static const _maximumGridWidth = 365.0;
  static const _minimumRowHeight = 65.0;
  ...
  final Map<MateoTextController, String> _canonicalValues = {};
  final Map<MateoTextController, VoidCallback> _controllerListeners = {};
  final Set<MateoTextController> _internallyEditedControllers = {};
  MateoTextController? _lastFocusedController;
  ...
  void _attachControllerListeners(
    Iterable<MateoTextController> controllers,
  ) {
  ...
  void _handleControllerChanged(MateoTextController controller) {
    if (controller.hasFocus) _lastFocusedController = controller;
    if (_localeName == null || _internallyEditedControllers.contains(controller)) {
      return;
    }
    _canonicalValues[controller] = _parseLocalized(controller.text);
  }
  ...
  MateoTextController get _activeController {
    for (final controller in widget.controllers) {
      if (controller.hasFocus) return controller;
    }

    final lastFocusedController = _lastFocusedController;
    if (lastFocusedController != null && widget.controllers.contains(lastFocusedController)) {
      return lastFocusedController;
    }
  ...
  bool _handleKeyPressed(MateoNumericKeypadKey key) {
    ...
    final canonical = _canonicalValues[controller] ?? _parseLocalized(controller.text);
  ...
  int _canonicalOffset(String displayedValue, int displayedOffset) {
    ...
    while (index < limit) {
      ...
      if (_westernDigit(codeUnit) != null || _localizedDigit(codeUnit) != null) {
        canonicalOffset += 1;
      }
      index += 1;
    }
  ...
  ({String value, int selection}) _normalizeCanonical(
    ...
    final fraction = decimalIndex < 0 ? null : value.substring(decimalIndex + 1);
    ...
    var removedZeroes = 0;
    while (removedZeroes < removableZeroes && integer.codeUnitAt(removedZeroes) == 0x30) {
      removedZeroes += 1;
    }
    if (removedZeroes > 0) {
  ...
  bool _isWithinDecimalLimit(String canonical) {
    ...
    return decimalIndex < 0 || canonical.length - decimalIndex - 1 <= widget.maxDecimals;
  }
  ...
  String _parseLocalized(String displayedValue) {
    ...
    while (index < displayedValue.length) {
      ...
      if (_groupingSeparator.isNotEmpty && displayedValue.startsWith(_groupingSeparator, index)) {
  ...
  String _formatCanonical(String canonical) {
    ...
    final decimalIndex = canonical.indexOf('.');
    final integer = decimalIndex < 0 ? canonical : canonical.substring(0, decimalIndex);
    final fraction = decimalIndex < 0 ? null : canonical.substring(decimalIndex + 1);
    final localizedInteger = _groupInteger(integer);

    return fraction == null ? localizedInteger : '$localizedInteger$_decimalSeparator$fraction';
  }

  String _groupInteger(String integer) {
    if (_rightmostGroupingSize == 0 || integer.length <= _rightmostGroupingSize) {
      return integer;
    }
  ...
  String _visibleValue(MateoNumericKeypadKey key) {
    return key == MateoNumericKeypadKey.decimalSeparator ? _decimalSeparator : key.rawValue ?? '';
  }
  ...
  String _semanticLabel(
    ...
    return switch (key) {
      ...
      MateoNumericKeypadKey.decimalSeparator => localizations.keyboardKeyNumpadDecimal,
      _ => key.rawValue!,
    };
  }
  ...
  Widget _buildKey(BuildContext context, MateoNumericKeypadKey key) {
    ...
    return MateoTap(
      ...
      semanticLabel: _semanticLabel(context, key),
      animation: MateoTapAnimationType.scale,
      onPressChanged: key == MateoNumericKeypadKey.backspace ? _handleBackspacePressChanged : null,
