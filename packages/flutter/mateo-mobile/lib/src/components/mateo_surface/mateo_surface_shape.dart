...
@immutable
final class MateoSurfaceShape {
  /// Creates a surface shape with no treatment of its rectangular boundary.
  const MateoSurfaceShape.none() : _border = const MateoRoundedShapeBorder(radius: 0), _radius = 0;

  /// Creates a capsule that adapts to the surface bounds.
  ///
  ...
  const MateoSurfaceShape.capsule() : _border = const MateoRoundedShapeBorder.capsule(), _radius = 0;

  /// Creates a rounded surface with the requested rounding [radius].
  // Keep the public argument name independent of private storage.
  // ignore: prefer_initializing_formals
  const MateoSurfaceShape.rounded({required double radius}) : _border = null, _radius = radius;
  ...
  @override
  bool operator ==(Object other) => other is MateoSurfaceShape && border == other.border;

  /// The hash of this surface treatment.
  @override
  int get hashCode => Object.hash(MateoSurfaceShape, border);
}
